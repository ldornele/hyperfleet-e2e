...
7. [Force-delete a single stuck nodepool without affecting parent cluster](#test-title-force-delete-a-single-stuck-nodepool-without-affecting-parent-cluster)
...
## Test Title: Force-delete a single stuck nodepool without affecting parent cluster

### Description

This test validates that force-deleting a single stuck nodepool removes only that nodepool while leaving the parent cluster unaffected and in a healthy Reconciled state. This confirms that force-delete is scoped to the target resource and does not cascade upward.
...
2. HyperFleet API and HyperFleet Sentinel services are deployed and running successfully
3. The adapters defined in testdata/adapter-configs are all deployed successfully
4. At least one nodepool adapter is configured in `h.Cfg.Adapters.NodePool`
...
curl -X POST ${API_URL}/api/hyperfleet/v1/clusters/{cluster_id}/nodepools \
  ...
  -d @testdata/payloads/nodepools/nodepool-request.json
```
- Wait for both to reach `Reconciled: True`

**Expected Result:**
- Cluster and nodepool are both `Reconciled: True`

#### Step 2: Scale down nodepool adapter and soft-delete the nodepool

**Action:**
- Scale the pre-deployed nodepool adapter to 0 replicas
- Send DELETE request for the nodepool only (not the cluster):
```bash
curl -X DELETE ${API_URL}/api/hyperfleet/v1/clusters/{cluster_id}/nodepools/{nodepool_id}
```
...
- Nodepool remains stuck in Finalizing because the adapter is down

#### Step 3: Force-delete the nodepool

**Action:**
```bash
curl -X POST ${API_URL}/api/hyperfleet/v1/clusters/{cluster_id}/nodepools/{nodepool_id}/force-delete \
  -H "Content-Type: application/json" \
  -d '{"reason": "E2E test: nodepool adapter unavailable"}'
```

**Expected Result:**
- Force-delete returns HTTP 204
- Nodepool is hard-deleted (GET returns HTTP 404)
...
**Expected Result:**
- Parent cluster returns HTTP 200
- `deleted_time` is null (cluster was NOT deleted)
- `Reconciled` condition is `True` (cluster is still healthy)

#### Step 5: Cleanup resources

**Action:**
- Scale nodepool adapter back to 1 replica
- Delete the test cluster via `CleanupTestCluster`

**Expected Result:**
- Nodepool adapter is restored
